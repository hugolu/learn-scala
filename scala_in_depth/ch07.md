...
Type class 是確保一個型別順某個抽象接口的機制。Type class 當初是作為 Haskell 語言的一個語言特性流行起來的，在 Scala 是通過**高階型別**和**隱式解析**來實現的。
...
另一種解法方案是不定義型別 `FileLike[T <: FileLike[T]]`，而是定義 `FileLike[T]`，新特徵能夠與任何 `T` 交互，把它當作 `File` 而不需要繼承關係。這種風格的特徵叫做 type class。

### 7.3.1 作為型別類別的 FileLike (FileLike as a type class)

type class 在 Scala 的存在形式如下：
- 一個高階的 type class trait 用作給定型別的 accessor 或 utility 函式庫。一個與 trait 同名的物件包含各種型別的 type class trait 的默認實現
- 需要使用 utility 函式庫裡提供的方法可以用 type class trait 作為上下文綁定以確保只有實現了 type class trait 的型別才能被使用，並保留原始類型
...
- 對於型別 `T` 沒有任何限制。`FileLike` type class trait 能用於任何型別 `T`
- 所有方法都接受一個 `T` 型別的參數。`FileLike` type class 沒有設計成另一個類別的**包裝器**，而是那個類別的數據或狀態的**訪問器** (accessor)

這使我們可以在通用地處理一個型別的同時，保留其原始型別。
...
```
- `def synchronize[F: FileLike, T: FileLike](from: F, to: T)...` 使用上下文綁定語法，等同於為給定型別的 `FileLike` 定義一個隱式參數
- `val fromHelper = implicitly[FileLike[F]]` 用隱式方法查找 `FileLike` 參數
- 每個使用 `T` 或 `F` 的方法呼叫都使用了 `FileLike` type class

作用於兩個 `java.io.File` 的物件，編譯器抱怨找不到 `FileLike[java.io.File]` 型別的隱形值 >>> `synchronize` 方法需要一個 `java.io.File` 的 type trait 實現
...
- 抽象分離
    - type class 創建新的抽象，允許其他型別適配或被適配到該型別
    - 當創建新抽象，用於已存在且無法修改的型別時，這個特性特別有價值
- 可組合性
    - 上下文邊界語法可以用於指定多個型別
    - 在寫新方法時可以很容易要求一些型別必須存在。這比要求某個抽象接口或抽象接口的組合要靈活多
- 可覆蓋
    - 可以利用隱式系統覆蓋型別默認的實現
- 型別安全
